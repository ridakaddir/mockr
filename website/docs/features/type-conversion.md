...
[Home](/) > [Features](index) > Type Conversion
...
- **`from`** -- a <code v-pre>{{ref:...}}</code> token that resolves to the source data
...
When applied to the directory via <code v-pre>{{ref:stubs/deployments/endpoint-1/?template=stubs/templates/traffic-split.json}}</code>, this produces:
```json
[{"dep-a": 60}, {"dep-b": 40}]
```
...
| `"from"` is not a <code v-pre>{{ref:...}}</code> token | <code v-pre>$as "from" value must be a {{ref:...}} token</code> |
...
5. Regular <code v-pre>{{ref:...}}</code> resolution
